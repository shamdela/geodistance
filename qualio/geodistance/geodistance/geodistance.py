...
class Location:
    ...
    value to store its proximity from Dublin
    """
...
class GreatCircleDistance:
    ...
    def calculate_distance(self, lat, lon):
        ...
        lat2 = math.radians(DUBLIN_LOC[0])
        lon2 = math.radians(DUBLIN_LOC[1])

        # Great-circle distance - Haversine
        lat_radians = lat2 - lat1
        lon_radians = lon2 - lon1

        a = (math.sin(lat_radians / 2) ** 2) + (math.cos(lat1) * math.cos(lat2)) * (math.sin(lon_radians / 2) ** 2)
    ...
    def process_json_data(self, json_city_records):
        ...
        for record in json_city_records:
            # find first json record
            location = json_city_records[record]
            ...
            if distance <= KMS_RADIUS:
                ...
                list_of_cities.append(c)    # add city to list of cities

        # sort list by city attribute
        list_of_cities.sort(key=lambda x: x.city)
